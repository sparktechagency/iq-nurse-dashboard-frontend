...
interface UpdatePatientAssessmentModalProps {
  visible: boolean;
  onCancel: () => void;
...
const UpdatePatientAssessmentModal: React.FC<UpdatePatientAssessmentModalProps> = ({ visible, onCancel, onSubmit, initialData }) => {
        ...
        <Form.Item label="Syndromes">
          <div className="border border-gray-300 rounded-lg overflow-hidden">
            <div className="max-h-80 overflow-y-auto">
              {syndromes.map((s, idx) => (
                <div className={`flex gap-2 items-center p-3 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`} key={idx}>
                  <Input placeholder="Assessment" value={s.asessment} onChange={e => handleSyndromeChange(idx, 'asessment', e.target.value)} className="flex-1" />
                  <Input placeholder="Normal" value={s.normal} onChange={e => handleSyndromeChange(idx, 'normal', e.target.value)} className="flex-1" />
                  <Input placeholder="Abnormal" value={s.abnormal} onChange={e => handleSyndromeChange(idx, 'abnormal', e.target.value)} className="flex-1" />
                  {syndromes.length > 1 && (
                    <Button icon={<DeleteOutlined />} danger onClick={() => handleRemoveSyndrome(idx)} className="flex-shrink-0" />
                  )}
                </div>
              ))}
            </div>
            <div className="border-t border-gray-300 p-2 bg-gray-50">
              <Button icon={<PlusOutlined />} onClick={handleAddSyndrome} className="w-full">Add Row</Button>
            </div>
          </div>
        </Form.Item>
...
export default UpdatePatientAssessmentModal;
